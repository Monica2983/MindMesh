...
import { moderateContent, CALLING_CONTRACT, getBalance, parseErrorMsg } from "../utils/utils";
...
export const SOCAIL_MEDIA_Provider = ({ children }) => {
  ...
  try {
    ...
    const account = await checkIfWalletConnected();
    if (account) {
      // Moderate the content before creating the post
      const result = await moderateContent(description);
      if (!result.allowed) {
        alert(result.reason);  // Alert if content is not allowed
        return;
      }
  ...
  try {
    ...
    // Use the postID passed as an argument
    const transaction = await SOCIAL_CONTRACT.likePost(Number(postID)); // Use postID here
    await transaction.wait();

    setCount(count + 1);  // Update count (you may want to adjust this depending on your logic)
    setLoader(false);
  ...
  try {
    ...
    const account = await checkIfWalletConnected();
    if (account) {
      const result = await moderateContent(_comment);
      if (!result.allowed) {
        alert(result.reason);  // 🔔 Alert offensive comment
        setLoader(false);
        return;
      }
